...
const areaImageSchema = z.object({
  index: z.number().int().min(0).max(200),
  imageUrl: z.string().trim().max(1200),
  imagePublicId: z.string().trim().max(300),
  imageAlt: z.string().trim().max(300),
});

/**
 * Which activity areas the homepage shows, and their photographs.
 *
 * Saved by one action rather than two so the operator gets a single Save
 * button: choosing a card and giving it a picture is one job. Both the
 * selection and the images are language independent — the card list is the
 * same length and order in each document — so both languages are written.
 * The card copy itself is edited elsewhere and is left untouched.
 */
export async function saveHomepageAreas(form: FormData) {
  const session = await requireAdmin();
  const locale = z.enum(["en", "tr"]).parse(text(form, "locale"));
  ...
  try {
    parsedImages = JSON.parse(text(form, "images") || "[]");
    parsedSelection = JSON.parse(text(form, "values") || "[]");
  } catch {
    throw new Error("Faaliyet alanı bilgileri okunamadı. Sayfayı yenileyip tekrar deneyin.");
  }
  const images = z.array(areaImageSchema).max(200).parse(parsedImages);
  const areas = z
    .array(z.number().int().min(0).max(200))
    .max(HOMEPAGE_SECTION_LIMIT)
    .parse(parsedSelection);
  ...
  await db.transaction(async (tx) => {
    for (const row of rows) {
      ...
      const programs = document.programs as { cards: Array<Record<string, unknown>> };
      programs.cards = programs.cards.map((card, index) => {
      ...
      document.homepage = { ...readHomepageSelection(document.homepage), areas };
  ...
  await audit(session.user.id, "save", "homepage_areas", locale);
  refreshPublic("/", "/areas-of-work");
  redirect(`/admin/homepage?locale=${locale}&saved=1`);
}
